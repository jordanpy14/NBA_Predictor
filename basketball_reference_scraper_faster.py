...
async def get_html_requester(browser, url, selector, sleep=5, retries=3):
    ...
    context = await browser.new_context(
        ignore_https_errors=True, proxy={"server": proxy_server}
    )
    page = await context.new_page()
    ...
    for i, player_stats in enumerate(top_players.itertuples(index=False), start=1):
        for stat in player_stats._fields:
            ...
            if stat in column_mapping:
                ...
                safe_stat_name = stat
            flattened_stats[f"{quarter}_player{i}_{name}_{safe_stat_name}"] = getattr(
                player_stats, stat
            )
...
async def scrape_season(browser, season):
    ...
    for i, url in enumerate(tqdm(standings_pages[1:], desc=f"Scraping {season}")):
        ...
        links = soup.find_all("a")
        box_scores = [
            f"https://www.basketball-reference.com{l.get('href')}"
            for l in links
            if l.get("href")
            and "boxscores" in l.get("href")
            and "html" in l.get("href")
        ]
        ...
        for url_box in tqdm(box_scores, desc=f"Scraping {season_date}"):
            ...
            game_teams = pd.DataFrame(
                {
                    ...
                    "date": [
                        pd.to_datetime(os.path.basename(url_box)[:8], format="%Y%m%d")
                    ],
                }
            )
            ...
            for index, team in enumerate(teams):
                for quarter in ["q1", "q2", "h1"]:
                    ...
                    team_basic = team_basic.rename(
                        lambda x: f"{quarter}_team_{name}_{x}".replace(
                            "%", "Per"
                        ).replace("+/-", "plus_minus")
                    )
                    ...
                    players_basic["MP_float"] = players_basic["MP"].apply(
                        format_minutes
                    )
                    # Sort by the new MP_float column and flatten the top 5 player stats
                    sorted_players = players_basic.sort_values(
                        by="MP_float", ascending=False
                    )
                ...
                name = "home" if index == 0 else "away"
                team_basic = team_basic.rename(
                    lambda x: f"game_team_{name}_{x}".replace("%", "Per").replace(
                        "+/-", "plus_minus"
                    )
                )
                team_basic_transposed = team_basic.to_frame().transpose()
                ...
                name = "home" if index == 0 else "away"
                team_advanced = team_advanced.rename(
                    lambda x: f"game_team_{name}_{x}".replace("%", "Per").replace(
                        "+/-", "plus_minus"
                    )
                )
            ...
            team_record = pd.DataFrame(
                {
                    "team_home_record": [team_record[0][1]],
                    "team_away_record": [team_record[1][1]],
                }
            )
        ...
        all_games.to_csv(
            f"data/{season}/{season_date}_all_games_summary.csv", index=False
        )
